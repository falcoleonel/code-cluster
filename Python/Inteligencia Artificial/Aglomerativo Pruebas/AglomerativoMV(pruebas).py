...
class Aglomerativo:
    ...
    def Ward(self, ClusterA, ClusterB):
        """
        Regresa la distancia definida por el incremento en la suma de errores cuadrados
        """
        # promedios
        pro1 = np.mean(ClusterA.puntos, axis=0).tolist()
        pro2 = np.mean(ClusterB.puntos, axis=0).tolist()
        ...
        Dist = (dis(pro1, pro2))
        mulP = len(ClusterA.puntos)*len(ClusterB.puntos)
        sumaP = len(ClusterA.puntos)+len(ClusterB.puntos)
        Res = (mulP/sumaP)*sqrt(Dist)
        print("\n     Distancia definida por el incremento en la suma del error cuadrado de los grupos:",
              ClusterA.indicador, " y ", ClusterB.indicador, "es de:", Res)
        return Res
