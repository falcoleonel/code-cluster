...
public class Imagen extends BufferedImage{
    ...
    public void draw_fill_Rect(int x0, int y0, int x1, int y1, Color c) {
    ...
    public void scanline(int newColor) {

        boolean inside = false;
        int oldColor = getColor(1,1);
        
        for (int y = 0; y < this.getWidth(); y++) {
            for (int x = 0; x < this.getWidth(); x++) {
        
                if(getColor(x,y)!=oldColor && !inside) inside =true;
                if(getColor(x,y)!=oldColor && inside) inside =false;

                if(x>this.getWidth()) return;
                if(y>this.getHeight()) return;
                
                if(inside) setRGB(x, y, newColor);
