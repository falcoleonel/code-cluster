...
public class Clock extends JFrame implements Runnable {
    ...
    public void paint(Graphics g) {
        if (background == null) {
            ...
            ImageIcon bg1 = new ImageIcon("src/body9.png");
    ...
    // private int manecillaSegundero=60;
	// private int bgHand=60;
	// private int smHand=40;
    ...
    public void update(Graphics g) {
        ...
        if (cal.get(Calendar.MINUTE) != min) {
			...
			// angle1 =angle12(hour - 3);
			// x1 =getx(angle1,smHand);
			// y1 =gety(angle1,smHand);
			// gbuffer.drawLine(getWidth()/2, getHeight()/2,(getHeight()/2)+(int)x1,(getHeight()/2)+(int)y1);


			// angle2 =angle60(min - 15);
			// x2 =getx(angle2,bgHand);
			// y2 =gety(angle2,bgHand);
        ...
        g2d2.drawImage(eyes, (x4 -250)/20, (y4 / 12), this); 
